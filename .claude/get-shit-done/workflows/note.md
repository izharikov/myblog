...
- **Global scope**: `D:/work/blog/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/work/blog/.claude/notes/`)
...
2. Glob `D:/work/blog/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/work/blog/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/work/blog/.claude/notes/` — works in any directory
